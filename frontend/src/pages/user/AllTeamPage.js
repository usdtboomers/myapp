...
const AllTeamPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchAllTeam = async () => {
      setLoading(true);
      try {
        const res = await api.get(`/user/all-team/${user.userId}?page=${currentPage}&limit=${entriesPerPage}&search=${search}`);
        ...
        // 🔥 FIX: Ab ye direct backend se Poori Team ka count uthayega, sirf page ka nahi!
        setStats({
          totalTeam: res.data.totalTeamCount || 0,
          activeTeam: res.data.activeCount || 0 // Yahan backend se direct Active Count aayega
        });
  ...
  const currentItems = sortedTeam; 

  // 🔥 FIX: Aage Peeche (Next/Prev) jane ka function theek kiya
  const handleNext = () => {
      ...
      <div style={{ overflowX: "auto" }}>
        <table style={styles.table}>
          ...
          <tbody>
            {loading ? (
              <tr>
                <td colSpan="7" style={styles.emptyCell}>⏳ Loading team data...</td>
              </tr>
