...
function ReferralTree() {
  ...
  const handleLoadTree = async () => {
    ...
    try {
      const [treeRes, allTeamRes] = await Promise.all([
        axios.get(`http://143.198.205.94:5000/api/user/tree/${userId}`),
        axios.get(`http://143.198.205.94:5000/api/user/all-team/${userId}`),
      ]);
        ...
        (treeRes.data.children || []).map(async (child) => {
          try {
            const teamRes = await axios.get(
              `http://143.198.205.94:5000/api/user/all-team/${child.userId}`
            );
            return {
              ...child,
              expanded: false,
  ...
  const handleExpandNode = async (node) => {
    ...
    try {
      const [treeRes, allTeamRes] = await Promise.all([
        axios.get(`http://143.198.205.94:5000/api/user/tree/${node.userId}`),
        axios.get(`http://143.198.205.94:5000/api/user/all-team/${node.userId}`),
      ]);
