...
const TopUpModal = ({ onClose, onTopUpSuccess }) => {
  ...
  const fetchUser = async () => {
    try {
      const res = await axios.get(`http://143.198.205.94:5000/api/user/${userId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleTopUp = async () => {
    ...
    try {
      await axios.put(
        `http://143.198.205.94:5000/api/user/topup/${Number(userId)}`,
      ...
      const refreshedRes = await axios.get(`http://143.198.205.94:5000/api/user/${userId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
