...
const startTelegramBot = () => {
    ...
    // 4 Din ke 4 alag messages ka array (Sahi Sequence + HTML Formatting)
    const promoMessages = [
        // Day 1 (Aapka naya wala pura message)
    ...
    // Cron Schedules
    const schedules = {
        promo: isTesting ? '* * * * *' : '0 7 * * *',       // Test: Har 1 min | Real: Subah 7:00 AM
        withdrawal: isTesting ? '* * * * *' : '1 0 * * *'   // Test: Har 1 min | Real: Raat 12:01 AM
    };
    ...
    cron.schedule(schedules.promo, async () => {
        try {
            ...
            // HTML parse mode lagaya hai format ke liye
            ...
            // Next message ke liye index update (0, 1, 2, 3 phir wapas 0)
    ...
    console.log(`🤖 Telegram Bot is running in ${isTesting ? 'TESTING (1 Min Interval)' : 'PRODUCTION'} mode...`);
};
